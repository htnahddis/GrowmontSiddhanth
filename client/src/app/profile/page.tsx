...
const ProfilePage = () => {
    ...
    try {
      ...
      if (salesRes.ok) {
        ...
        setProductSales(salesData.map((s: any) => ({
          id: String(s.id),
          productId: s.product_display || s.product,
          clientName: s.client_name || "Unknown",
          createdDate: s.date,
          priority: "High",
          category: s.product_display || "Unknown",
          amount: parseFloat(s.amount),
          categoryCount: 1,
          representatives: ["/avatar.png"]
        })));
      }
  ...
  const getPriorityColor = (priority: string) => {
    ...
    if (priorityLower.includes("high")) return "text-red-500";
    if (priorityLower.includes("medium")) return "text-yellow-500";
    if (priorityLower.includes("low")) return "text-green-500";
    return "text-gray-500";
  };

  const getPriorityBadgeColor = (priority: string) => {
    const priorityLower = priority.toLowerCase();
    if (priorityLower.includes("high")) return "bg-red-100 text-red-700 border-red-200";
    if (priorityLower.includes("medium")) return "bg-yellow-100 text-yellow-700 border-yellow-200";
    if (priorityLower.includes("low")) return "bg-green-100 text-green-700 border-green-200";
    return "bg-gray-100 text-gray-700 border-gray-200";
  };

  const getReminderIconBg = (type: string) => {
    if (type === "CORPORATE") return "bg-blue-100 text-blue-600";
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-[#2D8A4E] text-white px-6 py-2.5 rounded-lg hover:bg-[#236b3d] transition-colors shadow-md"
            >
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                <div className="flex gap-3 mb-6">
                  ...
                  <button
                    ...
                    className={`px-6 py-2 rounded-full text-sm font-medium transition ${activeTab === "interactions"
                      ? "bg-[#2D8A4E] text-white"
                      : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                      }`}
                  >
                  ...
                  <div className="space-y-4">
                    {interactions.length === 0 ? (
                      ...
                      interactions.map((interaction) => (
                        <div
                          key={interaction.id}
                          className="bg-gray-50 rounded-lg p-4 hover:shadow-md transition border-l-4 border-blue-500"
                        >
                  ...
                  <div className="space-y-4">
                    {reminders.length === 0 ? (
                      ...
                      reminders.map((reminder) => (
                        <div
                          key={reminder.id}
                          className="bg-white rounded-lg p-4 hover:shadow-md transition border-l-4 border-green-500"
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-4 flex-1">
                              <div className={`w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 ${getReminderIconBg(reminder.type)}`}>
                                <svg
                                  className="w-6 h-6"
                                  fill="none"
                                  stroke="currentColor"
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mt-6 max-w-2xl">
...
function ChangePasswordForm() {
  ...
  return (
    <form onSubmit={handleChangePassword} className="space-y-4">
      {message && (
        <div className="text-green-600 bg-green-50 border border-green-200 p-3 rounded-lg text-sm">
          {message}
        </div>
      )}
      {error && (
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-700 bg-gray-50 focus:ring-2 focus:ring-[#2D8A4E] focus:border-transparent outline-none"
          required
        />
      </div>
      <div>
      ...
      <button
        type="submit"
        className="bg-[#2D8A4E] text-white px-6 py-2 rounded-lg hover:bg-[#236b3d] transition-colors text-sm font-medium"
      >
        Update Password
      </button>
    </form>
